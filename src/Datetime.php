...
class Datetime
{
    ...
    {
        return easter_date( (is_null($year) ? date('Y') : $year));
    }
     ...
     * @throws \Exception
     */
    static public function getPeriode($startDate, $endDate = null, $nbMoisGlissant = false)
    {
        $dtStart = Datetime::getInstance();
        $dtEnd = Datetime::getInstance();
        
        $dtStart->set_date($startDate);
        $dtEnd->set_date(is_null($endDate) ? $startDate : $endDate);
        
        /*
         * Test si mois glissant
         */
        if ($dtStart->equals($dtEnd) === 0 && $nbMoisGlissant) {
            $dtStart->sub_month($nbMoisGlissant - 1);
        }
        
        if ($dtStart->equals($dtEnd) === 0) {
            $lib_periode = $dtStart->format('%B %Y');
            $shortlib_periode = $dtStart->format('%b %Y');
        }
        else {
            $lib_periode = $dtStart->format('%B %Y') . ' - ' . $dtEnd->format('%B %Y');
            $shortlib_periode = $dtStart->format('%b %Y') . ' - ' . $dtEnd->format('%b %Y');
        }
        
        $result = [
            'lib'      => $lib_periode,
            'shortlib' => $shortlib_periode,
            'start'    => $dtStart,
            'end'      => $dtEnd,
        ];
    ...
    /**
     * Test si la date est férié
     *
     * @param null|string $date
     *
     * @return bool
     */
    public function EstFerie($date = null)
    {
        if (!is_null($date)) {
            if ( $this->set_date($date) === false ){
                throw new \Exception("bad initialization: " . var_export($date, 1));
            }
        }
        return array_key_exists($this->format("%Y%m%d"), $this->joursferies);
    ...
    {
        if (is_null($date)) {
            return $this->set_timestamp(time());
        }
        elseif (is_numeric($date)) {
            return $this->set_timestamp($date);
        }
        ...
        elseif (preg_match(self::REGEX_SEM, $date, $dateSplitted)) {
            $baseDt = new \DateTime();
            $baseDt->setISOdate($dateSplitted['Y'], $dateSplitted['S']);
            return $this->set_date($baseDt->getTimestamp());
        }
    ...
    /**
     * Retourne la liste des jours fériés
     * @return array
     */
    public function getListeJoursFeries(){
